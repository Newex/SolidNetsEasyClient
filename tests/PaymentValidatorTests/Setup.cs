...
{
    public static PaymentRequest DefaultPayment(bool company = true, int webHooks = 2, int paymentConfigurations = 1, int paymentMethods = 1, int orderItems = 5)
    {
        return Fakes.RandomPayment(company, termsUrl: true, webHooks, paymentConfigurations, paymentMethods, orderItems);
    }

    public static PaymentRequest WithCheckout(this PaymentRequest payment, Func<Checkout, Checkout> setCheckout)
    {
        return payment with
        {
...
#nullable enable
    public static PaymentRequest WithShippingAddress(this PaymentRequest payment, Func<ShippingAddress, ShippingAddress?> setShippingAddress)
    {
    ...
    public static PaymentRequest WithConsumer(this PaymentRequest payment, Func<Consumer, Consumer?> setConsumer)
    {
        var consumer = payment.Checkout.Consumer ?? Fakes.RandomConsumer(payment.Checkout.Consumer?.Company is not null);
        return payment with
        {
    ...
    public static PaymentRequest WithNotifications(this PaymentRequest payment, int webHooks, Func<Notification, Notification?>? setNotification = null)
    {
    ...
    public static PaymentRequest WithPaymentConfigurations(this PaymentRequest payment, Func<IEnumerable<PaymentMethodConfiguration>, IEnumerable<PaymentMethodConfiguration>> setPaymentConfigurations)
    {
